...
const TrendingCards = ({name, price, symbol, thumbnail} : TrendindCardProps) => {
    return (
        <Box horizontal className='bg-neutral-800 h-[180px] min-w-[400px] rounded-3xl'>
            <Box className='w-[180px] bg-zinc-700/20 min-h-full rounded-xl p-8'>
            ...
            <Box className='w-1/2 py-4 px-4 gap-2'>
                <Text className='text-slate-300 text-xl'>
                    {name}
                    ({symbol})</Text>
                <Text className='text-slate-300 text-xl'>PRICE (USD):<br/>{
                    price.toFixed(8)
                }</Text>
            </Box>
